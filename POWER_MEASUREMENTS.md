...
- one combined plot: `..._combined.jpg`
- one plot per channel: `..._<channel>.jpg`
...
2. **Moves** the CSV + JPGs into the wiki at
   `assets/power/<WikiBasename>/<YYMMDD_HHMM>/`.
...
Only the Python standard library is used (Python 3.9+). The tool is idempotent:
